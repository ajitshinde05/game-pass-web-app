    ...
    <div class="cont-padding">
      ...
      >
        ...
        <div class="d-flex justify-content-center mb-64 mtt-40">
          <b-button variant="primary" class="mt-2 mr-1" @click="close(amount)">
            Add
          </b-button>
        </div>
      </b-modal>
  ...
  export default {
    ...
    methods: {
      ...
      close(val) {
        ...
        const game = {
          gameId: this.currentActiveGame,
          userName: user.username,
          userId_numberSelected_oneMinute: {
            [user.username]: [
              {
                betted_numbers: this.selectedKey,
                betted_amount: this.amount,
              },
            ],
          },
        };
        this.addUserDataToCurrentActiveGame(game);
        this.amount = 0;
      ...
      async addUserDataToCurrentActiveGame(game) {
        this.isLoading = true;
        const res = await new APIService().api(
          {
            method: 'post',
            url: `game/OneMinuteGameController/addUserDataToCurrentActiveGame`,
          },
          { ...game },
          {},
        );
      ...
      async getCurrentOneMinuteRunningGame() {
        ...
        if (res && res.gameId) {
          this.currentActiveGame = res.gameId;
          this.targetDate = moment(res.endTime);
